...
def top2gating(logits: torch.Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    # combine_weights = combine1_sec + combine2_sec # OOM problem for large models since
    # Use in-place addition to save memory since combine1_sec and combine2_sec are not needed after this point.
...
def basetop2gating(logits: torch.Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    """Implements Top2Gating on logits."""
    # NOTE(msb) softmax requires FP32: https://docs.nvidia.com/deeplearning/performance/mixed-precision-training/
    gates = F.softmax(logits, dim=1, dtype=torch.float)
    ...
    num_experts = gates.shape[1]
    # capacity = 2S/E
    capacity = (2 * num_tokens + num_experts - 1) // num_experts
...
class BaseTop2Gate(torch.nn.Module):
    """Gate module which implements Top2Gating as described in Gshard_.
    ::
